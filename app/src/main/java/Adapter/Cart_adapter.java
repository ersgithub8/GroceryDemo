...
//        if (dbHandler.isInCart(map.get("title"))) {
            Glide.with(activity)
        ...
        SharedPreferences.Editor editor = activity.getSharedPreferences("1", MODE_PRIVATE).edit();
        editor.putString("1", map.get("title"));
        editor.apply();

            holder.tv_reward.setText(map.get("rewards"));
            holder.tv_price.setText(activity.getResources().getString(R.string.tv_pro_price) + " " +
                    map.get("unit") +"  "+ activity.getResources().getString(R.string.currency)+ map.get("price")  );
            holder.tv_contetiy.setText(map.get("qty"));
            Double items = Double.parseDouble(dbHandler.getInCartItemQty(map.get("c_id")));

            // Double stock = Double.parseDouble(dbHandler.getInCartItemQty(map.get("in_stock")));
            ...
            holder.iv_minus.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    if (qty > 0) {
                        ...
                        dbHandler.update(String.valueOf(qty),map.get("c_id")) ;

                        Cursor res=dbHandler.viewAll();
                        while (res.moveToNext()){
                            price1=res.getString(6);
                            quan=res.getString(1);
                            rp=res.getString(7);
                        }
...
//                    dbHandler.setCart(map, Float.valueOf(holder.tv_contetiy.getText().toString()));
                        ...
                        Double items = Double.parseDouble(dbHandler.getInCartItemQty(map.get("c_id")));
                        //Double store = Double.parseDouble(dbHandler.getInCartItemQty(map.get("storeid")));
                        Double price = Double.parseDouble(price1);
                        Double reward = Double.parseDouble(rp);
                        holder.tv_total.setText("" + price * items);
                        holder.tv_reward.setText("" + reward * items);
                        holder.tv_contetiy.setText(String.valueOf(qty));
                        updateintent();
                    ...
                    if (holder.tv_contetiy.getText().toString().equalsIgnoreCase("0")) {
                        dbHandler.removeItemFromCart(map.get("c_id"));
