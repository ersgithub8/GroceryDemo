...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    public static final String COLUMN_CART = "c_id";
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String exe = "CREATE TABLE IF NOT EXISTS " + CART_TABLE
                + "(" + COLUMN_CART + " integer primary key autoincrement, "
    ...
    public boolean setCart(HashMap<String, String> map, Float Qty) {
        ...
        if (isInCart(map.get(COLUMN_CART)))
        {

            db.execSQL("update " + CART_TABLE + " set " + COLUMN_QTY + " = '" + Qty + "' where " + COLUMN_CART + "=" + map.get(COLUMN_CART));
            return false;

        }
        else {
            ContentValues values = new ContentValues();
            values.put(COLUMN_CART, map.get(COLUMN_CART));
    ...
    public boolean isInCart(String id) {
        ...
        String qry = "Select *  from " + CART_TABLE + " where " + COLUMN_CART + " = " + id;
    ...
    public String isTileInCart(String id) {
        ...
        {
            String qry1 = "Select *  from " + CART_TABLE + " where " + COLUMN_TITLE + " = " + id;
    ...
    public String getProductUnit(String id) {
        ...
        db = getReadableDatabase();
            ...
            String qry1 = "Select *  from " + CART_TABLE + " where " + COLUMN_CART + " = " + id;
    ...
    public String getInCartItemQty2(String id) {
        if (isInCart2(id)) {
            db = getReadableDatabase();
            String qry = "Select *  from " + CART_TABLE + " where " + COLUMN_STOCK+ " = " + id;
            Cursor cursor = db.rawQuery(qry, null);
            cursor.moveToFirst();
            return cursor.getString(cursor.getColumnIndex(COLUMN_QTY));
        } else {
    ...
    public boolean isInCart2(String id) {
        db = getReadableDatabase();
        String qry = "Select *  from " + CART_TABLE + " where " + COLUMN_STOCK + " = " + id;
        Cursor cursor = db.rawQuery(qry, null);
        cursor.moveToFirst();
    ...
    public ArrayList<HashMap<String, String>> getCartAll() {
        ...
        for (int i = 0; i < cursor.getCount(); i++) {
            ...
            map.put(COLUMN_CART, cursor.getString(cursor.getColumnIndex(COLUMN_CART)));
    ...
    public void removeItemFromCart(String id) {
        db = getReadableDatabase();
        db.execSQL("delete from " + CART_TABLE + " where " + COLUMN_CART + " = " + id);
    }
    ...
    public boolean update(String Qty,String cartid){
        db = getWritableDatabase();
        db.execSQL("update " + CART_TABLE + " set " + COLUMN_QTY + " = '" + Qty + "' where " + COLUMN_CART + "=" + cartid);
        return true;
    }
