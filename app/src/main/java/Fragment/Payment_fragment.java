...
public class Payment_fragment extends Fragment {
    ...
    private static String TAG = Payment_fragment.class.getSimpleName();
     String unit;
     String unit_value;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        lat = prefs.getFloat("8",1);
         lang= prefs.getFloat("9",1);//"No name defined" is the default value.
        //u8Toast.makeText(getActivity(), String.valueOf(lat+"  "+lang), Toast.LENGTH_SHORT).show();


        SharedPreferences preferencess = getActivity().getSharedPreferences("5", Context.MODE_PRIVATE);

       // SharedPreferences preferencess = this.getActivity().getSharedPreferences("5", Context.MODE_PRIVATE);
         user_idname= preferencess.getString("5", "No name defined");
       // Toast.makeText(getActivity(), user_idname, Toast.LENGTH_SHORT).show();


        SharedPreferences preferences = this.getActivity().getSharedPreferences("1", MODE_PRIVATE);

        Storeid= preferences.getString("1", "No name defined");//"No name defined" is the default value.
       ...
       // Toast.makeText(getActivity(), getuser_id, Toast.LENGTH_SHORT).show();
        ...
        house_no=getArguments().getString("house_no");
        deliveryCharge = getArguments().getString("delivery_charge");
...
//                    confirm.setEnabled(false);
        ...
        if (items.size() > 0) {
            ...
            for (int i = 0; i < items.size(); i++) {
                ...
                try {
                    ...
                    jObjP.put("delivery_charge",deliveryCharge);
                    jObjP.put("house_no",house_no);
                    unit = map.get("unit");
                    unit_value = map.get("unit_value");

                  //  Toast.makeText(getActivity(), unit, Toast.LENGTH_SHORT).show();
                  //  Toast.makeText(getActivity(), house_no, Toast.LENGTH_SHORT).show();
                    passArray.put(jObjP);
                } catch (JSONException e) {
                    e.printStackTrace();
                }
                //Toast.makeText(getActivity(),String.valueOf(passArray), Toast.LENGTH_SHORT).show();
            }
            ...
            if (ConnectivityReceiver.isConnected()) {
                ...
                //Toast.makeText(getActivity(),getstore_id, Toast.LENGTH_SHORT).show();
                makeAddOrderRequest(getdate, gettime, getuser_id, getlocation_id, getstore_id, passArray,unit,unit_value);
               //makeGetcity1(getdate, gettime, getuser_id, getlocation_id, getstore_id, passArray,unit,unit_value);
    ...
    private void makeAddOrderRequest(String date, String gettime, String userid, String
      ...
      //  Toast.makeText(getActivity(), date+gettime+userid+location+store_id+passArray+unit+unit_value, Toast.LENGTH_SHORT).show();
        String tag_json_obj = "json_add_order_req";
        Map<String, String> params = new HashMap<String, String>();
        params.put("date", date);
        params.put("time", gettime+"-"+gettime);
        ...
        params.put("delivery_method",delivery_method);
        params.put("delivery_charge",deliveryCharge);
        params.put("unit", unit);
        params.put("unit_value",unit_value);
        params.put("house_no",lat+","+lang);


       // Toast.makeText(getActivity(),date+gettime+ delivery_method, Toast.LENGTH_SHORT).show();
...
//                    msg_arb=response.getString("data_arb");
//                        Toast.makeText(getActivity(),msg_arb, Toast.LENGTH_SHORT).show();
...
//        loading.show();
        ...
        if (NetworkConnection.connectionChecking(getActivity())) {
            ...
            StringRequest postReq = new StringRequest(Request.Method.POST, BaseURL.COUPON_CODE,
                ...
                protected Map<String, String> getParams() throws AuthFailureError {
                    ...
                    params.put("user_id", user_idname);
                 //   Toast.makeText(getActivity(), userrr_idname, Toast.LENGTH_SHORT).show();
            ...
            location, String store_id, JSONArray passArray,String unit,String unit_value)
    {
       ...
       params.put("data", passArray.toString());
        ...
        params.put("location", location);
        params.put("store_id", store_id);
        //Toast.makeText(getActivity(),String.valueOf(passArray), Toast.LENGTH_LONG).show();
